...
% % Define patch size (e.g., 16x16)
% patchSize = 16; 
% 
% % Reshape to image format
% imageLMS = CalFormatToImage(triLMSCalFormat, Disp.m, Disp.n);
% imageLMS_plate = CalFormatToImage(triLMSCalFormat_plate, Disp.m, Disp.n);
% 
% % Get image size
% [imgH, imgW, ~] = size(imageLMS);
% 
...
% correctedImage = zeros(size(imageLMS));
% correctedImage_plate = zeros(size(imageLMS_plate));

% for i = 1:numPatchesY
%     for j = 1:numPatchesX
%         % Extract patch
%         xStart = (j-1) * patchSize + 1;
%         xEnd = j * patchSize;
%         yStart = (i-1) * patchSize + 1;
%         yEnd = i * patchSize;
% 
%         patchLMS = imageLMS(yStart:yEnd, xStart:xEnd, :);
%         patchLMS_plate = imageLMS_plate(yStart:yEnd, xStart:xEnd, :);
% 
%         % Convert patch to [3 x numPixels] format
%         patchLMS_reshaped = ImageToCalFormat(patchLMS);
%         patchLMS_plate_reshaped = ImageToCalFormat(patchLMS_plate);
% 
%         % Apply color correction locally
%         switch (method)
%             case 'linTransform'
%                 DispPatch = Disp;
%                 DispPatch.m = patchSize;
%                 DispPatch.n = patchSize;
%                 patchLMS_corrected = colorCorrectionOptimize(patchLMS_reshaped, renderType, lambda_var, DispPatch, bScale);
%                 patchLMS_plate_corrected = colorCorrectionOptimize(patchLMS_plate_reshaped, renderType, lambda_var, DispPatch, bScale);
%             case 'easyPCA'
%                 patchLMS_corrected = colorCorrectionEasyPCA(patchLMS_reshaped, renderType, Disp, bScale);
%                 patchLMS_plate_corrected = colorCorrectionEasyPCA(patchLMS_plate_reshaped, renderType, Disp, bScale);
%             case 'hardPCA'
%                 numPCs = 2;
%                 patchLMS_corrected = colorCorrectionHardPCA(patchLMS_reshaped, numPCs, Disp);
%                 patchLMS_plate_corrected = colorCorrectionHardPCA(patchLMS_plate_reshaped, numPCs, Disp);
%         end
% 
%         % Convert corrected patch back to image format
%         patchCorrectedImage = CalFormatToImage(patchLMS_corrected, patchSize, patchSize);
%         patchCorrectedImage_plate = CalFormatToImage(patchLMS_plate_corrected, patchSize, patchSize);
% 
%         % Place corrected patch back into the image
%         correctedImage(yStart:yEnd, xStart:xEnd, :) = patchCorrectedImage;
%         correctedImage_plate(yStart:yEnd, xStart:xEnd, :) = patchCorrectedImage_plate;
%     end
% end
% 
% % Convert corrected image back to [3 x nPixels] format
% triLMScalFormatCorrected = ImageToCalFormat(correctedImage);
% triLMScalFormatCorrected_plate = ImageToCalFormat(correctedImage_plate);
